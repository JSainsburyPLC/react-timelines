'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _getPrototypeOf = require('babel-runtime/core-js/object/get-prototype-of');

var _getPrototypeOf2 = _interopRequireDefault(_getPrototypeOf);

var _classCallCheck2 = require('babel-runtime/helpers/classCallCheck');

var _classCallCheck3 = _interopRequireDefault(_classCallCheck2);

var _createClass2 = require('babel-runtime/helpers/createClass');

var _createClass3 = _interopRequireDefault(_createClass2);

var _possibleConstructorReturn2 = require('babel-runtime/helpers/possibleConstructorReturn');

var _possibleConstructorReturn3 = _interopRequireDefault(_possibleConstructorReturn2);

var _inherits2 = require('babel-runtime/helpers/inherits');

var _inherits3 = _interopRequireDefault(_inherits2);

var _react = require('react');

var _react2 = _interopRequireDefault(_react);

var _propTypes = require('prop-types');

var _propTypes2 = _interopRequireDefault(_propTypes);

var _Header = require('./Header');

var _Header2 = _interopRequireDefault(_Header);

var _Body = require('./Body');

var _Body2 = _interopRequireDefault(_Body);

var _Now = require('./Marker/Now');

var _Now2 = _interopRequireDefault(_Now);

var _Pointer = require('./Marker/Pointer');

var _Pointer2 = _interopRequireDefault(_Pointer);

var _getMouseX = require('../../utils/getMouseX');

var _getMouseX2 = _interopRequireDefault(_getMouseX);

var _formatDate = require('../../utils/formatDate');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var Timeline = function (_Component) {
  (0, _inherits3.default)(Timeline, _Component);

  function Timeline(props) {
    (0, _classCallCheck3.default)(this, Timeline);

    var _this = (0, _possibleConstructorReturn3.default)(this, (Timeline.__proto__ || (0, _getPrototypeOf2.default)(Timeline)).call(this, props));

    _this.handleMouseMove = _this.handleMouseMove.bind(_this);
    _this.handleMouseEnter = _this.handleMouseEnter.bind(_this);
    _this.handleMouseLeave = _this.handleMouseLeave.bind(_this);
    _this.handleScroll = _this.handleScroll.bind(_this);
    _this.handleResize = _this.handleResize.bind(_this);

    _this.state = {
      pointerX: 0,
      pointerVisible: false,
      pointerHighlighted: false,
      scrollLeft: 0
    };
    return _this;
  }

  (0, _createClass3.default)(Timeline, [{
    key: 'componentDidMount',
    value: function componentDidMount() {
      if (this.props.stickyHeader) {
        window.addEventListener('resize', this.handleResize);
        this.props.getMarkerOffset(this.timeline);
        this.props.getTimelineWidth(this.timeline);
      }
    }
  }, {
    key: 'componentWillReceiveProps',
    value: function componentWillReceiveProps(nextProps) {
      if (this.props.isHeaderSticky !== nextProps.isHeaderSticky) {
        var scrollLeft = this.timeline.scrollLeft;
        this.setState({ scrollLeft: scrollLeft });
      }
    }
  }, {
    key: 'componentDidUpdate',
    value: function componentDidUpdate(prevProps) {
      if (prevProps.isOpen !== this.props.isOpen) {
        this.props.getTimelineWidth(this.timeline);
      }
    }
  }, {
    key: 'componentWillUnmount',
    value: function componentWillUnmount() {
      if (this.props.stickyHeader) {
        window.removeEventListener('resize', this.handleResize);
      }
    }
  }, {
    key: 'handleMouseMove',
    value: function handleMouseMove(e) {
      this.setState({ pointerX: (0, _getMouseX2.default)(e) });
    }
  }, {
    key: 'handleMouseLeave',
    value: function handleMouseLeave() {
      this.setState({ pointerHighlighted: false });
    }
  }, {
    key: 'handleMouseEnter',
    value: function handleMouseEnter() {
      this.setState({ pointerVisible: true, pointerHighlighted: true });
    }
  }, {
    key: 'handleScroll',
    value: function handleScroll() {
      var _this2 = this;

      requestAnimationFrame(function () {
        var scrollLeft = _this2.timeline.scrollLeft;
        _this2.setState({ scrollLeft: scrollLeft });
      });
    }
  }, {
    key: 'handleResize',
    value: function handleResize() {
      var _this3 = this;

      requestAnimationFrame(function () {
        _this3.props.getTimelineWidth(_this3.timeline);
      });
    }
  }, {
    key: 'render',
    value: function render() {
      var _this4 = this;

      var _props = this.props,
          now = _props.now,
          time = _props.time,
          timebar = _props.timebar,
          tracks = _props.tracks,
          isHeaderSticky = _props.isHeaderSticky,
          headerHeight = _props.headerHeight,
          timelineVisualWidth = _props.timelineVisualWidth,
          getHeaderHeight = _props.getHeaderHeight;
      var _state = this.state,
          pointerX = _state.pointerX,
          pointerVisible = _state.pointerVisible,
          pointerHighlighted = _state.pointerHighlighted,
          scrollLeft = _state.scrollLeft;

      return _react2.default.createElement(
        'div',
        { className: 'timeline', ref: function ref(timeline) {
            _this4.timeline = timeline;
          }, onScroll: isHeaderSticky && this.handleScroll },
        _react2.default.createElement(
          'div',
          { className: 'timeline__content', style: { width: time.timelineWidth + 'px' } },
          now && _react2.default.createElement(_Now2.default, { now: now, visible: true, time: time }),
          _react2.default.createElement(_Pointer2.default, {
            x: pointerX,
            visible: pointerVisible,
            highlighted: pointerHighlighted,
            text: (0, _formatDate.getDayMonth)(time.fromX(pointerX))
          }),
          _react2.default.createElement(
            'div',
            null,
            _react2.default.createElement(_Header2.default, {
              time: time,
              timebar: timebar,
              onMove: this.handleMouseMove,
              onEnter: this.handleMouseEnter,
              onLeave: this.handleMouseLeave,
              width: time.timelineWidth,
              isSticky: isHeaderSticky,
              height: headerHeight,
              visualWidth: timelineVisualWidth,
              scrollLeft: scrollLeft,
              getHeight: getHeaderHeight
            })
          ),
          _react2.default.createElement(_Body2.default, { time: time, tracks: tracks })
        )
      );
    }
  }]);
  return Timeline;
}(_react.Component);

Timeline.propTypes = {
  now: _propTypes2.default.instanceOf(Date),
  time: _propTypes2.default.shape({}).isRequired,
  timebar: _propTypes2.default.shape({}).isRequired,
  tracks: _propTypes2.default.arrayOf(_propTypes2.default.shape({})),
  getHeaderHeight: _propTypes2.default.func,
  getMarkerOffset: _propTypes2.default.func,
  getTimelineWidth: _propTypes2.default.func,
  isHeaderSticky: _propTypes2.default.bool,
  stickyHeader: _propTypes2.default.bool,
  headerHeight: _propTypes2.default.number,
  timelineVisualWidth: _propTypes2.default.number,
  isOpen: _propTypes2.default.bool
};

exports.default = Timeline;
module.exports = exports['default'];